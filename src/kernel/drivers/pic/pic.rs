...
/// * `divider` - Timer divider (1, 2, 4, 8, 16, 32, 64, 128)
...
impl IoApic {
    ...
    /// 
    /// # Arguments
    /// * `base_addr` - Physical base address of I/O APIC (from MADT or default)
    ...
    unsafe fn read(&self, reg: u32) -> u32 {
        ...
        write_volatile_u32(self.base_addr, reg);
        // Read data from window
        read_volatile_u32(self.base_addr + 0x10)
    }
    ...
    unsafe fn write(&self, reg: u32, value: u32) {
        ...
        write_volatile_u32(self.base_addr + 0x10, value);
    }
    
    /// Get I/O APIC ID
    pub unsafe fn get_id(&self) -> u32 {
        (self.read(IOAPIC_REG_ID) >> 24) & 0xFF
    }
    
    /// Get I/O APIC version and max redirection entries
    pub unsafe fn get_version(&self) -> (u8, u8) {
        let version_reg = self.read(IOAPIC_REG_VERSION);
        let version = (version_reg & 0xFF) as u8;
        let max_entries = ((version_reg >> 16) & 0xFF) as u8;
        (version, max_entries)
    }
    
    /// Read a redirection entry (64-bit value)
    /// 
    /// # Arguments
    /// * `irq` - IRQ number (0-23 typically)
    pub unsafe fn read_redirection_entry(&self, irq: u8) -> u64 {
    ...
    /// Write a redirection entry (64-bit value)
    /// 
    /// # Arguments
    /// * `irq` - IRQ number (0-23 typically)
    /// * `entry` - 64-bit redirection entry value
    pub unsafe fn write_redirection_entry(&self, irq: u8, entry: u64) {
        let low_reg = IOAPIC_REG_REDTBL_BASE + (irq as u32 * 2);
        let high_reg = low_reg + 1;
    ...
    /// Configure a redirection entry for an IRQ
    /// 
    /// # Arguments
    /// * `irq` - IRQ number (0-23 typically)
    /// * `vector` - Interrupt vector (32-255)
    /// * `apic_id` - Target Local APIC ID
    ...
    pub unsafe fn mask_irq(&self, irq: u8) {
        ...
        self.write_redirection_entry(irq, entry | IOAPIC_MASKED);
    }
...
/// Create I/O APIC instance with default base address
pub unsafe fn create_default_ioapic() -> IoApic {
    IoApic::new(IOAPIC_DEFAULT_BASE)
}
...
pub struct MadtLocalApic {
    ...
    pub processor_id: u8,
    pub apic_id: u8,
    pub flags: u32,  // Bit 0: Processor enabled
}
...
pub struct MadtInterruptOverride {
    pub header: MadtEntryHeader,
    pub bus: u8,  // 0 = ISA
    pub source: u8,  // IRQ source
    pub global_system_interrupt: u32,
    pub flags: u16,  // MPS INTI flags
}
